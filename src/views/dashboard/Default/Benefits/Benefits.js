...
//Collections
import { collBenefits } from 'store/collections';
import * as Msg from 'store/message';
import * as LogMsg from 'store/logsMessages';
import { generateId } from 'utils/idGenerator';
import { titles, inputLabels } from './Benefits.texts';
...
export default function Benefits() {
  ...
  const getData = async () => {
    const list = [];
    const querySnapshot = await getDocuments(collBenefits);
  ...
  const handleCreate = () => {
    if (!type || !ci || !name || !phone || !address || !city || !mail) {
      ...
      createDocument(collBenefits, ide, obj);
  ...
  const handleEdit = () => {
    if (!type || !ci || !name || !phone || !address || !city || !mail) {
      ...
      updateDocument(collBenefits, id, obj);
  ...
  const handleDelete = () => {
    ...
    createDocument(collHistBusiness, histId, obj);
    deleteDocument(collBenefits, id);
